...
class PosDemoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (empty($consultations)) {
            $this->command->error('No consultations found. Run ConsultationDemoSeeder first.');
            return;
        }

        // ── 4. Determine how many more sales to seed ────────────────
        $existingSales = DB::table('sales')->count();
        $toSeed        = self::TOTAL - $existingSales;

        if ($toSeed <= 0) {
            $this->command->info('PosDemoSeeder: already at target (' . number_format($existingSales) . ' sales).');
            return;
        }
        ...
        $txnBase    = $existingSales;

        $this->command->info("Seeding " . number_format($toSeed) . " POS sales (target: " . number_format(self::TOTAL) . ')...');
        ...
        $this->command->info('PosDemoSeeder done — ' . number_format(DB::table('sales')->count()) . ' total sales.');
        $this->command->info('Sale items: ' . number_format(DB::table('sale_items')->count()));
        $this->command->info('Payments:   ' . number_format(DB::table('payment_transactions')->count()));
    }
